      ...
      <table class="table table-sm table-striped">
        ...
        <tbody>
          @foreach($tours as $tour)
            <tr>
              <td>{{ $tour->getTranslatedName() ?? $tour->name }}</td>
              <td>
                <span class="badge bg-secondary">
                  {{ $tour->tourType->getNameTranslatedAttribute() ?? $tour->tourType->name ?? '—' }}
                </span>
              </td>
              <td>
          ...
          <div class="card-header bg-dark text-white">
            <h6 class="mb-0">
              ...
              <span class="badge bg-info ms-2">
                {{ __('m_bookings.capacity.badges.base') }}: {{ $tour->max_capacity ?? '—' }}
              </span>
            </h6>
          </div>
                    ...
                    <td>
                      <form action="{{ route('admin.tours.schedule.update-pivot-capacity', [$tour, $schedule]) }}"
                        ...
                        <input type="number"
                               ...
                               min="1"
                               max="999"
                               placeholder="{{ __('m_bookings.capacity.messages.empty_placeholder', ['capacity' => $tour->max_capacity ?? 15]) }}"
                      ...
                      <small class="text-muted d-block mt-1">
                        {{ __('m_bookings.capacity.messages.empty_placeholder', ['capacity' => $tour->max_capacity ?? 15]) }}
                      </small>
                    </td>
                    <td>
