...
<style>
  ...
  /* ====== Selector nuevo (chip) ====== */
  .image-select{ position:absolute; top:.45rem; left:.45rem; z-index:15; }
  .image-select input[type="checkbox"]{ position:absolute; opacity:0; width:0; height:0; pointer-events:none; }
  .image-check-label{
  ...
  .image-select input[type="checkbox"]:checked + .image-check-label::after{ content:'✓'; }
  ...
  /* ====== Stats ====== */
  .stats-bar{ display:flex; gap:.6rem; align-items:center; flex-wrap:wrap; margin-bottom:var(--space-3); }
  .stat-item{
...
const confirm = (title, text) => Swal.fire({
  ...
  confirmButtonText:i18n.confirm_btn, cancelButtonText:i18n.cancel_btn, confirmButtonColor:'#dc3545'
});
...
function initCaptions() {
  ...
  document.querySelectorAll('.caption-input').forEach(input => {
    ...
    input.addEventListener('input', () => {
      clearTimeout(timer);
      status.innerHTML = '<i class="fas fa-clock text-muted"></i> ' + (i18n.saving || '{{ __('m_tours.image.ui.saving_fallback', [], false) ?? 'Guardando...' }}');
...
// Flash
@if(session('success')) toast('success', @json(__('m_tours.image.done') . ': ' . session('success'))); @endif
@if(session('error')) toast('error', @json(__('m_tours.image.ui.error_title') . ': ' . session('error'))); @endif
@if($errors->any())
  Swal.fire({ icon:'error', title:@json(__('m_tours.image.ui.error_title')), html:'<ul style="text-align:left;">@foreach($errors->all() as $e)<li>{{$e}}</li>@endforeach</ul>' });
@endif
</script>
@endpush
