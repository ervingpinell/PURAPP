    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    @foreach($trashedTours as $tour)
                    @php
                    $daysLeft = max(0, 30 - now()->diffInDays($tour->deleted_at));
                    $tourName = $tour->getTranslatedName() ?: $tour->name ?: 'Sin nombre';
                    @endphp
                    <tr>
                        <td>
                            <strong>{{ $tourName }}</strong>
                            @if($tour->slug)
                            <br><small class="text-muted">{{ $tour->slug }}</small>
                            @endif
                        </td>
                        ...
                        <td>{{ $tour->deleted_at->format('d/m/Y H:i') }}</td>
                        <td class="text-center">
                        ...
                        <td class="text-center">
                            <div class="btn-group btn-group-sm">
                                @can('restore-tours')
                                <form action="{{ route('admin.tours.restore', $tour->tour_id) }}" method="POST" class="d-inline restore-form">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form action="{{ route('admin.tours.forceDelete', $tour->tour_id) }}" method="POST" class="d-inline force-delete-form">
                                    @csrf
                                    @method('DELETE')
