...
<script>
    $(function() {
        ...
        function loadSchedules(tourId, preselect = '{{ old('
            schedule_id ', optional($booking->detail)->schedule_id) }}') {
        ...
        function loadLanguages(tourId, preselect = '{{ old('
            tour_language_id ', $booking->tour_language_id) }}') {
        ...
        $('#btn-verify-promo').on('click', function() {
            ...
            if (!code) {
                $('#promo-feedback').removeClass('text-success').addClass('text-danger').text('{{ __('
        ...
        if ($('#hotel_id').val() === 'other' || '{{ old('
            is_other_hotel ', optional($booking->detail)->is_other_hotel) ? 1 : 0 }}' === '1') {
        ...
        const initialTourId = '{{ old('
        tour_id ', $booking->tour_id) }}';
        if (initialTourId) {
            $('#tour_id').val(initialTourId).trigger('change.select2');
            loadSchedules(initialTourId, '{{ old('
                schedule_id ', optional($booking->detail)->schedule_id) }}');
            loadLanguages(initialTourId, '{{ old('
                tour_language_id ', $booking->tour_language_id) }}');
            bootstrapCategoriesFromInit();
