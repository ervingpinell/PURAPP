...
<head>
  ...
  <style>
    ...
    :root{
      --green-dark:#1A5229; --green-base:#2E8B57; --gray-light:#f0f2f5;
      --text-color:#333; --font-heading:'Montserrat',sans-serif; --font-body:'Lora',serif;
    }
    body{ font-family:var(--font-body); font-size:14px; background:var(--gray-light); color:var(--text-color); margin:0; padding:40px 0; line-height:1.6; }
    h2{ text-align:center; color:var(--green-dark); margin:0 0 20px; font-family:var(--font-heading); font-size:28px; letter-spacing:1.2px; text-transform:uppercase; }
    .report-container{ max-width:850px; margin:0 auto; padding:0 20px; }
    .booking-page{ background:#fff; border:1px solid #e0e0e0; border-radius:10px; padding:22px 20px; page-break-inside: avoid; }
    .booking-section{ border:1px solid #e7e7e7; border-radius:8px; padding:16px; margin-bottom:16px; }
    .section-title{ font-weight:700; color:var(--green-base); margin-bottom:12px; font-family:var(--font-heading); font-size:17px; }
    .data-item{ margin-bottom:7px; display:flex; align-items:baseline; }
    .data-item strong{ color:var(--green-dark); min-width:140px; display:inline-block; font-family:var(--font-heading); font-weight:600; font-size:13px; }
    .data-item strong::after{ content: ':'; margin-left:2px; }
    .data-item span,.data-item small{ font-family:var(--font-body); font-size:14px; color:var(--text-color); margin-left:8px; }
    .data-item small{ font-size:12px; color:#777; }
    .line-separator{ border-top:1px dashed #c0c0c0; margin:12px 0; }
    .total{ font-weight:700; color:var(--green-dark); text-align:right; font-size:18px; font-family:var(--font-heading); margin-top:10px; }
    .category-breakdown{ background:#f9fdf9; border-radius:6px; padding:10px; margin:8px 0; }
    .category-item{ display:flex; justify-content:space-between; padding:4px 0; font-size:13px; }
    .summary-global{ background:#eaf5ed; border:1px solid var(--green-base); border-radius:10px; padding:18px; margin-top:18px; page-break-inside: avoid; }
    .summary-global .data-item{ font-size:15px; font-family:var(--font-heading); font-weight:600; color:var(--green-dark); }
    .summary-global .data-item strong{ min-width:150px; }
    .page-break{ page-break-after: always; }
    @media print{
      body{ background:none; padding:0; margin:0; }
      .report-container{ max-width:initial; padding:0; }
      .booking-page{ border:1px solid #cfcfcf; border-radius:0; padding:14mm 12mm; margin:0; }
    ...
    @foreach($bookings as $booking)
      @php
        ...
        $meetingPoint = (!$hasHotel && !empty($detail->meeting_point_id))
          ? ($detail->meeting_point_name ?? optional($detail->meetingPoint)->name ?? $EM)
          : null;
