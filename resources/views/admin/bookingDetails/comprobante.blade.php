...
<head>
  ...
  <style>
    ...
    .comprobante-container { max-width:680px; margin:auto; background:#fff; border:none; border-radius:10px; padding:30px; box-shadow:0 8px 20px rgba(0,0,0,0.08); }
    ...
    .datos-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px 30px; }
    .dato { display:flex; flex-direction:column; padding:8px 0; border-bottom:1px dashed #ddd; }
    .dato:last-of-type, .datos-grid > div:nth-last-child(2):nth-child(odd) { border-bottom:none; }
    .dato strong { color:var(--green-base); margin-bottom:6px; font-size:13px; font-family:var(--font-heading); font-weight:600; }
    .dato span { font-size:15px; color:var(--text-color); }
    .dato small { font-size:12px; color:#777; }
    ...
    @media (max-width:600px){ body{padding:15px;} .comprobante-container{padding:20px;} .datos-grid{grid-template-columns:1fr; gap:10px;} h2{font-size:24px;} .total{font-size:18px;} }
    @media print { body{background:none; padding:0; margin:0;} .comprobante-container{box-shadow:none; border:1px solid #ccc; border-radius:0; margin:1cm auto; max-width:initial;} }
  </style>
</head>
<body>
  <div class="comprobante-container">
    ...
    @php
      ...
      // Subtotal base y promo
      $subtotal = ($aPrice * $aQty) + ($kPrice * $kQty);

      // toma promo desde relación directa o redención
      ...
      $labelSurcharge = \Illuminate\Support\Facades\Lang::has('receipt.surcharge_label') ? __('receipt.surcharge_label') : 'Recargo';
      $adjustLabel = $op === 'add' ? $labelSurcharge : $labelDiscount;
    ...
    <div class="datos-grid">
      <div class="dato"><strong>{{ __('receipt.code') }}</strong><span>{{ $reserva->booking_reference }}</span></div>
      <div class="dato"><strong>{{ __('receipt.client') }}</strong><span>{{ optional($reserva->user)->full_name }}</span><small>({{ optional($reserva->user)->email }})</small></div>
      <div class="dato"><strong>{{ __('receipt.tour') }}</strong><span>{{ $tour->name }}</span></div>
      <div class="dato"><strong>{{ __('receipt.booking_date') }}</strong><span>{{ $bookingDate }}</span></div>
      <div class="dato"><strong>{{ __('receipt.tour_date') }}</strong><span>{{ $tourDate }}</span></div>
      <div class="dato"><strong>{{ __('receipt.schedule') }}</strong><span>{{ $horario }}</span></div>
      <div class="dato"><strong>{{ __('receipt.hotel') }}</strong><span>{{ $hotel }}</span></div>
      <div class="dato"><strong>{{ $meetingPointLabel }}</strong><span>{{ $meetingPointName }}</span></div>
      <div class="dato"><strong>{{ __('receipt.status') }}</strong><span>{{ $statusT }}</span></div>
    </div>

    <div class="line-separator"></div>

    <div class="datos-grid">
      <div class="dato"><strong>{{ str_replace(':count', (string)$aQty, __('receipt.adults_x')) }}</strong><span>${{ number_format($aPrice * $aQty, 2) }}</span></div>
      <div class="dato"><strong>{{ str_replace(':count', (string)$kQty, __('receipt.kids_x')) }}</strong><span>${{ number_format($kPrice * $kQty, 2) }}</span></div>
      <div class="dato"><strong>{{ __('receipt.people') }}</strong><span>{{ $aQty + $kQty }}</span></div>
    </div>
    ...
    <div class="total-section">
      <div style="text-align:right;">
        ...
        @if($promo && $delta > 0)
          <div style="color: {{ $op === 'add' ? '#b45309' : 'green' }};">
            <strong>{{ $adjustLabel }} ({{ $promo->code }}):</strong>
            {{ $op === 'add' ? '+' : '-' }}${{ number_format($delta, 2) }}
          </div>
        @endif

        <div class="total mt-2">{{ __('receipt.total') }}: ${{ number_format($reserva->total, 2) }}</div>
      </div>
    </div>
