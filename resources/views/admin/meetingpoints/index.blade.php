        ...
        <form id="editForm" method="POST">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <input type="text" name="name" id="edit_name" class="form-control" placeholder="Opcional">
              <small class="text-muted">Si no lo cambias, se mantiene. El nombre por idioma se edita abajo.</small>
            </div>
            <div class="mb-3">
              <label class="form-label">Hora de recogida</label>
            ...
            <div class="mb-3">
              <label class="form-label">Descripción (base)</label>
              <input type="text" name="description" id="edit_description" class="form-control" placeholder="Opcional">
            </div>
            <div class="mb-3">
              <label class="form-label">URL de mapa</label>
            ...
            <div class="row g-2">
              ...
              <div class="col-sm-8 mb-2">
                <label class="form-label">Nombre (traducción) <span class="text-danger">*</span></label>
                <input type="text" name="t_name" id="edit_t_name" class="form-control" required>
              </div>
              <div class="col-12">
                <label class="form-label">Descripción (traducción)</label>
                <textarea name="t_description" id="edit_t_description" class="form-control" rows="2"></textarea>
                <small class="text-muted">
                  Si eliges el locale <strong>{{ strtoupper(config('app.fallback_locale','es')) }}</strong>, también se sincroniza con los campos base.
                </small>
              </div>
            </div>
          </div>
...
const showSuccess = (title, text = '') => {
  Swal.fire({ icon:'success', title, text, showConfirmButton:false, timer:2500, timerProgressBar:true });
};
const showError = (title, text = '') => {
  Swal.fire({ icon:'error', title, text, confirmButtonText:'Entendido', confirmButtonColor:'#3085d6' });
};
const confirmAction = (title, text, icon = 'warning') => {
  return Swal.fire({ title, text, icon, showCancelButton:true, confirmButtonColor:'#3085d6', cancelButtonColor:'#d33', confirmButtonText:'Sí, continuar', cancelButtonText:'Cancelar' });
};
...
if (editModalElement) {
  editModal = new bootstrap.Modal(editModalElement, { backdrop:true, keyboard:true, focus:true });
...
function fillTranslationFields(translations, locale) {
  const t = (translations && translations[locale]) ? translations[locale] : {name:'', description:''};
...
document.querySelectorAll('.edit-btn').forEach(btn => {
  btn.addEventListener('click', function() {
    ...
    const trans = (() => { try { return JSON.parse(this.dataset.translations || '{}'); } catch(e){ return {}; } })();
...
editForm.addEventListener('submit', function(e) {
  ...
  if (!tname) {
    return Swal.fire({ icon:'error', title:'Falta el nombre (traducción)', text:'Completa el campo de nombre traducido.' });
  }
  confirmAction('¿Guardar cambios?', 'Se actualizará el meeting point y la traducción seleccionada.', 'question')
...
document.querySelector('.create-form')?.addEventListener('submit', function(e){
  ...
  confirmAction('¿Crear nuevo meeting point?', name ? `Se creará "${name}"` : 'Se creará un nuevo punto de encuentro', 'question')
    .then(result => { if (result.isConfirmed) this.submit(); });
});
...
document.querySelectorAll('.toggle-btn').forEach(btn => {
  btn.addEventListener('click', function(e){
    ...
    confirmAction(isActive ? '¿Desactivar meeting point?' : '¿Activar meeting point?', isActive ? `"${name}" quedará inactivo` : `"${name}" quedará activo`, 'warning')
      .then(result => {
...
document.querySelectorAll('.toggle-btn-mobile').forEach(btn => {
  btn.addEventListener('click', function(e){
    ...
    const isActive = this.dataset.active === '1';
    const route = "{{ route('admin.meetingpoints.toggle', ':id') }}".replace(':id', id);
    confirmAction(isActive ? '¿Desactivar?' : '¿Activar?', isActive ? `"${name}" quedará inactivo` : `"${name}" quedará activo`, 'warning')
      .then(result => {
        if (result.isConfirmed) {
          const form = document.getElementById('toggleForm');
          form.action = route;
...
document.querySelectorAll('.delete-btn').forEach(btn => {
  btn.addEventListener('click', function(e){
    ...
    confirmAction('¿Eliminar meeting point?', `"${name}" será eliminado permanentemente. Esta acción no se puede deshacer.`, 'error')
      .then(result => {
        if (result.isConfirmed) {
...
document.querySelectorAll('.delete-btn-mobile').forEach(btn => {
  btn.addEventListener('click', function(e){
    ...
    const name = this.dataset.name || '';
    const route = "{{ route('admin.meetingpoints.destroy', ':id') }}".replace(':id', id);
    confirmAction('¿Eliminar?', `"${name}" será eliminado permanentemente. Esta acción no se puede deshacer.`, 'error')
      .then(result => {
        if (result.isConfirmed) {
          const form = document.getElementById('deleteForm');
          form.action = route;
