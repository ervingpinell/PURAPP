...
/* Stats */
.cap-stats{ display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:8px }
@media (max-width: 420px){ .cap-stats{grid-template-columns:repeat(2,1fr)} }
...
@media (max-width: 480px){
  .cap-actions-row .cap-btn{ flex:1 0 48%; }
  .cap-actions-row .cap-btn.ms-auto{ margin-left:0 !important; flex-basis:100%; }
}
...
(function(){
  ...
  const renderDetails = (container, res)=>{
    const rows = res?.data || [];
    if(!rows.length){ container.innerHTML=`<p class="text-muted mb-0 p-3">${T('operation_cancel')}</p>`; return; }
  ...
  list?.addEventListener('click', async (e)=>{
    ...
    if(e.target.closest('.js-det')){
      ...
      try{
        ...
        if(err?.name==='AbortError'){ bodyEl.innerHTML=`<p class="text-muted mb-0 p-3">${T('operation_cancel')}</p>`; }
        else { bodyEl.innerHTML=`<p class="text-danger mb-0 p-3">${T('cannot_load')}</p>`; }
      }finally{
    ...
    if(e.target.closest('.js-block')){
      ...
      const { isConfirmed } = await Swal.fire({
        ...
        confirmButtonText: T('yes_block'), cancelButtonText: T('cancel')
      });
