...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 wrap-center">
  <div>
    ...
    <p class="text-muted mb-0" style="font-size:.9rem">{{ $product->name }}</p>
  </div>
  <a href="{{ route('admin.products.images.pick') }}" class="btn btn-warning btn-sm">
...
@section('content')
@php
$images = $product->getRelation('images') ?? $product->images()->orderBy('position')->get();
$count = $images->count();
$isFull = $count >= $max;
@endphp
...
<div class="container-fluid py-3 wrap-center">
  ...
  <div class="stats-bar">
    ...
    <div class="stat-item">
      ...
      <div>
        <div class="stat-value">{{ $product->coverImage ? '1' : '0' }}</div>
      ...
      <div class="card shadow-sm image-card">
        ...
        <div class="caption-zone">
          <input
            ...
            value="{{ $image->caption }}"
            data-id="{{ $image->id }}"
            data-url="{{ route('admin.products.images.update', [$product, $image]) }}">
        ...
        <div class="card-actions">
          ...
          <button type="button" class="btn btn-success btn-compact w-auto flex-grow-1 set-cover"
            data-url="{{ route('admin.products.images.setCover', [$product, $image]) }}">
...
<form id="bulkDeleteForm" action="{{ route('admin.products.images.bulk-destroy', $product) }}" method="POST" style="display:none;">
  @csrf @method('DELETE')
  <input type="hidden" name="ids" id="bulkIds">
</form>
<form id="deleteAllForm" action="{{ route('admin.products.images.destroyAll', $product) }}" method="POST" style="display:none;">
  @csrf @method('DELETE')
</form>
