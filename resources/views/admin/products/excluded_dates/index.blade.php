...
<style>
  ...
  .al-day-header {
    ...
    align-items: center;
    gap: var(--gap);
    border: 1px solid var(--title-separator);
  }
...
<div class="card-header bg-dark text-white d-flex flex-wrap align-items-end gap-2 sticky-filters">
  <form id="filtersForm" method="GET" action="{{ route('admin.products.excluded_dates.index') }}" class="d-flex flex-wrap align-items-end gap-2 mb-0">
    <div>
  ...
  <div class="ms-auto d-flex align-items-end gap-2">
    ...
    <a class="btn btn-warning btn-sm js-view-blocked ms-3"
      href="{{ route('admin.products.excluded_dates.blocked', ['date' => $date, 'days' => $days, 'q' => $q]) }}">
      <i class="fas fa-lock me-1"></i> {{ __('m_bookings.excluded_dates.buttons.view_blocked') }}
    </a>

    <a class="btn btn-info btn-sm ms-2"
      href="{{ route('admin.products.capacity.index') }}">
...
<script>
  ...
  function tBlockAll() {
    return I18N.block_all_btn || I18N.fallback_block_all;
  }

  function tUnblockAll() {
    return I18N.unblock_all_btn || I18N.fallback_unblock_all;
  }
  ...
  const CAPACITY_URL = @json(route('admin.products.capacity.store'));
  ...
  (function() {
    ...
    function apply() {
      ...
      if (stickyBar) {
        stickyBar.classList.toggle('is-stuck', wantTop === tightTop);
      }
    }
  ...
  async function toggleOne(el, day, productId, scheduleId, want) {
    ...
    try {
      ...
      toast.fire({
        icon: 'success',
        title: available ? I18N.unblock : I18N.block
      });
    } catch (e) {
      console.error(e);
  ...
  function setCapacityForDay(day) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        toast.fire({
          icon: 'success',
          title: I18N.updated_count.replace(':count', updated)
        });
        setTimeout(() => window.location.reload(), 1000);
      }
    });
  ...
  function getDayCheckboxes(day) {
    return document.querySelectorAll(`.row-item[data-day="${day}"] .select-item`);
  }

  function getBlockCheckboxes(day, bucket) {
    return document.querySelectorAll(`#day-${day}-${bucket} .select-item`);
  }

  function areAllChecked(list) {
    const arr = Array.from(list);
    return arr.length > 0 && arr.every(cb => cb.checked);
  }
