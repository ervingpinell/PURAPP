...
return [
'id' => $it->itinerary_id,
'name' => $it->getTranslation('name', $currentLocale, false) ?? null,
'description' => $it->getTranslation('description', $currentLocale, false) ?? null,
'items' => $it->items->map(function ($item) use ($currentLocale) {
return [
'id' => $item->item_id,
'title' => $item->getTranslation('title', $currentLocale, false) ?? null,
'description' => $item->getTranslation('description', $currentLocale, false) ?? null,
];
})->values()->toArray(),
];
})->toArray();
...
<div class="container-fluid">
    ...
    <form id="tour-itinerary-form" method="POST" action="{{ route('admin.products.wizard.store.itinerary', $product) }}">
        ...
        <div class="action-buttons">
            ...
            <a href="{{ route('admin.products.itinerary.index') }}"
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="itinerary_id"
                                        ...
                                        @php
                                        $itName = $itinerary->getTranslation('name', $currentLocale, false)
                                        ?? __('m_tours.itinerary.fields.name') . ' #' . $itinerary->itinerary_id;
                                        @endphp
                                        <option value="{{ $itinerary->itinerary_id }}"
                                            {{ old('itinerary_id', $product->itinerary_id ?? '') == $itinerary->itinerary_id ? 'selected' : '' }}>
                                            {{ $itName }}
                                        </option>
                                        @endforeach
                                    </select>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <div class="pool-container">
                                                            ...
                                                            return $item->getTranslation('title', $currentLocale, false) ?? '';
                                                            });
                                                            @endphp

                                                            @forelse($availableItems as $item)
                                                            @php
                                                            $title = $item->getTranslation('title', $currentLocale, false) ?? '';
                                                            $desc = $item->getTranslation('description', $currentLocale, false) ?? '';
                                                            @endphp
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="card card-secondary">
                            ...
                            <div class="card-body">
                                ...
                                @php
                                $it = $product->itinerary;
                                $itName = $it->getTranslation('name', $currentLocale, false) ?? __('m_tours.itinerary.fields.name');
                                $itDesc = $it->getTranslation('description', $currentLocale, false) ?? null;
                                @endphp
