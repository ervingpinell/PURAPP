            ...
            <form action="{{ route('admin.taxes.update', $tax) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $tax->name) }}" required>
                        @error('name')<span class="invalid-feedback">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label for="code">{{ __('taxes.fields.code') }} <span class="text-danger">*</span></label>
                        <input type="text" name="code" id="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $tax->code) }}" required>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="type" id="type" class="form-control @error('type') is-invalid @enderror">
                                    <option value="percentage" {{ old('type', $tax->type) == 'percentage' ? 'selected' : '' }}>{{ __('taxes.types.percentage') }}</option>
                                    <option value="fixed" {{ old('type', $tax->type) == 'fixed' ? 'selected' : '' }}>{{ __('taxes.types.fixed') }}</option>
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" step="0.01" name="rate" id="rate" class="form-control @error('rate') is-invalid @enderror" value="{{ old('rate', $tax->rate) }}" min="0" required>
                                    <div class="input-group-append">
                                        <span class="input-group-text" id="rate-suffix">{{ $tax->type == 'percentage' ? '%' : '$' }}</span>
                                    </div>
                                </div>
                                <small class="form-text text-muted" id="rate-help">{{ $tax->type == 'percentage' ? 'Para porcentaje: ej 13 = 13%' : 'Monto fijo en dólares' }}</small>
                    ...
                    <div class="form-group">
                        ...
                        <select name="apply_to" id="apply_to" class="form-control @error('apply_to') is-invalid @enderror">
                            <option value="subtotal" {{ old('apply_to', $tax->apply_to) == 'subtotal' ? 'selected' : '' }}>{{ __('taxes.apply_to_options.subtotal') }}</option>
                            <option value="total" {{ old('apply_to', $tax->apply_to) == 'total' ? 'selected' : '' }}>{{ __('taxes.apply_to_options.total') }}</option>
                            <option value="per_person" {{ old('apply_to', $tax->apply_to) == 'per_person' ? 'selected' : '' }}>{{ __('taxes.apply_to_options.per_person') }}</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description', $tax->description) }}</textarea>
                        @error('description')<span class="invalid-feedback">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label for="sort_order">{{ __('taxes.fields.sort_order') }}</label>
                        <input type="number" name="sort_order" id="sort_order" class="form-control @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $tax->sort_order) }}" min="0">
