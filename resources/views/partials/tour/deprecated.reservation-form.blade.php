...
// ======= Global (settings DB -> config fallback)
...
<script>
  (function() {
    ...
    if (legacySum) {
      new MutationObserver(() => updateTotals()).observe(legacySum, {
        childList: true,
        subtree: true,
        characterData: true
      });
    ...
    const scheduleChoices = new Choices(scheduleSel, {
      ...
      itemSelectText: '',
      placeholder: true,
      placeholderValue: '-- ' + T.selectOption + ' --'
    });
    const langChoices = new Choices(langSelect, {
      searchEnabled: false,
      shouldSort: false,
      itemSelectText: ''
    });
    ...
    const meetingChoices = new Choices(meetingSel, {
      searchEnabled: true,
      shouldSort: false,
      itemSelectText: '',
      placeholder: true,
    ...
    const BASE_CHOICES = scheduleChoices._store.choices.filter(c => c.value !== '').map(c => ({
    ...
    const toggleOther = () => {
      ...
      if (isOther) {
        warnMsg && (warnMsg.style.display = '');
        otherInp && otherInp.setAttribute('required', 'required');
      } else {
        warnMsg && (warnMsg.style.display = 'none');
        if (otherInp) {
          otherInp.removeAttribute('required');
          otherInp.value = '';
        }
      }
    };
    ...
    addBtn.addEventListener('click', async () => {
      ...
      addBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>{{ __('
      adminlte::adminlte.add_to_cart ') }}';
      ...
      try {
        ...
        if (typeof data.count !== 'undefined' && window.setCartCount) {
          ...
          try {
            const cRes = await fetch(window.CART_COUNT_URL, {
              headers: {
                'Accept': 'application/json'
              }
            });
        ...
        await Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Error de red. Intenta nuevamente.',
          confirmButtonText: T.ok
        });
      } finally {
