...
/*
|--------------------------------------------------------------------------
| SEO locale routing (PÚBLICO)
| - Prefijo {locale} y SetLocale (URL::defaults(['locale'=>...]))
| - Nombres de ruta conservados (no rompemos blades)
| - Recuerda: LocaleRedirect debe ser GLOBAL en bootstrap/app.php
|--------------------------------------------------------------------------
*/

// Aceptamos solo estos locales en URL
Route::pattern('locale', 'es|en|fr|de|pt');

// Grupo PÚBLICO localizado
Route::prefix('{locale}')
    ...
    ->group(function () {
        ...
        // Cambiar idioma manteniendo ruta
        ...
        // ⬇️ Clave: usamos {id} porque en blades se llama route('tours.show', ['id' => $tourId])
        ...
        Route::get('/politicas/{policy}', [\App\Http\Controllers\PoliciesController::class, 'show'])->name('policies.show');
        ...
        // Reenviar verificación pública (anti-enumeración)
        ...
        Route::middleware(['auth', 'verified'])->group(function () {
            ...
            // Carrito (alias doble por compat)
...
/*
|--------------------------------------------------------------------------
| Admin (SIN localización, fuera del prefijo {locale})
|--------------------------------------------------------------------------
*/
Route::middleware(['auth', 'verified', 'can:access-admin'])
    ...
    ->group(function () {
        ...
        Route::middleware('2fa.admin')->group(function () {
            ...
            Route::post('policies/{policy}', [PolicyController::class, 'toggle'])->name('policies.toggle');
